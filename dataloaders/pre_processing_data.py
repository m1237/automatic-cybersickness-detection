...
def ecg_to_hr(ecg_data: np.ndarray):
    sample_rate = 130
    filtered = hp.filter_signal(ecg_data, cutoff=0.05, sample_rate=sample_rate, filtertype='notch')
    # resample the data. Usually 2, 4, or 6 times is enough depending on original sampling rate
    resampled_data = resample(filtered, len(filtered) * 2)
...
def bvp_df_to_hr_df(bvp_df: pd.DataFrame):
    bvp_numpy = bvp_df['bvp'].to_numpy()
    wd, m = hp.process(bvp_numpy, sample_rate=130)
    df_hr = pd.DataFrame()
    df_hr["timestamp"] = bvp_df["timestamp"]
    df_hr["hr"] = wd["hr"]
    return df_hr
...
if __name__ == '__main__':
    print(hr_to_ibi(
        ecg_to_heart_rate(json_to_dataframe("../data/train/1/Polar_h10/ECG.json", value_name='ecg'))))
    #
    # hr = (bvp_df_to_hr_df(json_to_dataframe("../Storage/training/elias/Empatica/BVP.json", value_name='bvp')))
    # print(hr)
    # # ECG
    # FILE_PATH_ECG = 'C:/Users/Marja/PyCharmProjects/git/lab-1-via-vr/collected_data/marlen/Polar_h10/ECG.json'
    # # neurokit(FILE_PATH_ECG)
    # heartpy(FILE_PATH_ECG)
    # # biospy()
    #
    # # ACC
    # FILE_PATH_ACC = 'C:/Users/Marja/PyCharmProjects/git/lab-1-via-vr/collected_data/barbara/Empatica/ACC.json'
