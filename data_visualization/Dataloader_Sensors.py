...
from data_processing.pre_processing_data import ecg_to_heart_rate, hr_to_ibi
from util import acc_json_to_dataframe, json_to_dataframe
from util import merge_dfs, fill_milliseconds, substitute_bools, generate_timeseries_data
...
def preprocess_sensor_data(sensors, dfs, buffer,
    ...
    for df in dfs:
        ...
        df['timestamp'] = df['timestamp'] + pd.Timedelta(hours=1)  # bc of different system times
    ...
    df[pc_keys] = df[pc_keys].replace([np.nan, -np.inf, np.inf], 0)
...
def get_sensor_data(path_training_data, name, buffer, with_orig=False):
...
def add_label(user_df_merged, label_df, buffer):
    ...
    for idx, cs_timestamp in label_df.iterrows():
        annotated_df = (user_df_merged['timestamp'] >= cs_timestamp["start"]) & (user_df_merged['timestamp'] <= cs_timestamp["end"])
...
if __name__ == '__main__':
    ...
    # df = get_sensor_data_for_all_persons_by_folder(f'../Storage/training')
    # print(df.shape)

    # parser.add_argument("--buffer", help="buffer for merging eyetracking/sensor data",
    #                     default='0.1 S')
    # parser.add_argument("--buffer_cs_timestamps", help="buffer for merging measurement data and cs timestamps",
    #                     default=500000)

    simple_buffer = '0.1 S'
    # Use all of them which ends with run
    preprocessed_data = get_sensor_data('../data/train', '1', simple_buffer)
